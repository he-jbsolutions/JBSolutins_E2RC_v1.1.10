...
namespace e2rcModel.BusinessLayer
{
    public class Inspection
    {
        ...
        public dynamic LocationDetails(long Location_ID)
        {
            ...
            if (dataset != null && dataset.Tables.Count > 0 && dataset.Tables[0].Rows.Count > 0)
            {
                ...
                return new
                {
                    ...
                    F2_ID = Convert.ToString(row["F2_ID"])
                };
            }
            return new { locationName = "", LocationID = 0 };
        }
        ...
        public dynamic InspectorDetails(long Inspector_ID)
        {
            ...
            if (dataset != null && dataset.Tables.Count > 0 && dataset.Tables[0].Rows.Count > 0)
            {
                ...
                return new
                {
                    ...
                    ContactNumber = Convert.ToInt64(row["MobileNumber"]),
        ...
        {
            DataSet dataset = new DAL().ExecuteStoredProcedure("sp_ClientNames_List", new object[] { "@Inspector_ID" }, new object[] { Inspector_ID });
            List<dynamic> InspectorsClientList = new List<dynamic>();
            if (dataset != null && dataset.Tables.Count > 0 && dataset.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in dataset.Tables[0].Rows)
                {
                    InspectorsClientList.Add(new
                    {
                        Client_ID = Convert.ToInt64(row["Client_ID"]),
                        Name = Convert.ToString(row["CompanyName"]),
                    });
                }
                return InspectorsClientList;
            }
            else
            {
                InspectorsClientList.Add(new { Client_ID = 0, Name = "" });
                return InspectorsClientList;
            }
        }


        public Inspection Single(long? Inspection_ID)
        {
        ...
        public dynamic getClientDetailsByInspectionId(long? Inspection_ID)
        {
            ...
            return new { UploadLogoPath = "", CompanyName = "" }; ;
        }
        ...
        public bool deleteStormDetails(long Storm_ID)
        {
            return new DAL().Update("sp_deleteStormID_CRUD",
                    new object[] { "@Storm_ID" },
                    new object[] { Storm_ID }
                    );
        }
